...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# MND HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on MND HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"MND_DEV{random.randint(10000, 99999)}"
...
def login():
    if request.method == 'POST':
        ...
        users = load_users()
        if username == 'GTMND' and password == users.get('GTMND', {}).get('password'):
    session['user'] = 'GTMND'
    session['role'] = 'admin'
...
if __name__ == '__main__':
    ...
    print("=" * 50 + "\n")
    app.run(debug=True, host='0.0.0.0', port=22018)